#!/usr/bin/env python3
"""Larix Music Reactive Lights v1.6.0 – beat-focused, Zigbee-safe, hot-reload"""
...
UPDATE_MS = max(int(O.get("update_interval_ms", 400)), 250)
...
class HA:
    ...
    def _req(self, method, path, **kw):
        try:
            r = self.s.request(method, f"{HA_URL}{path}", timeout=1.5, **kw)
            r.raise_for_status()
...
class Analyzer:
    ...
    def process(self, pcm, sens):
        ...
        beat = 0.0
        now = time.time()
        if len(self.eh) >= 10 and (now - self.last_beat) > 0.35:
    ...
    def ok(self, eid, bri, hue, sat, min_iv, force=False):
        ...
        if (now - pt) < min_iv:
            return False
        if abs(bri - pb) < 35 and abs(hue - ph) < 30 and abs(sat - ps) < 20:
            return False
        self._last[eid] = (bri, hue, sat, now)
        return True
...
def map_lights(ha, az, f, bands, mode, min_b, max_b, trans, iv):
    ...
    if mode == "auto":
        ...
        if az._hold_bri is None:
            ...
            az._hold_hue = base_hue
            az._hold_sat = base_sat
        az._hold_bri = int(0.85 * az._hold_bri + 0.15 * ambient)
        az._hold_hue = 0.9 * az._hold_hue + 0.1 * base_hue
        az._hold_sat = 0.9 * az._hold_sat + 0.1 * base_sat
        min_iv = max(1.0, iv * 3)
        for e in targets:
            if az.ok(e, az._hold_bri, az._hold_hue, az._hold_sat, min_iv, force=False):
                ha.set_lights([e], az._hold_bri, hs=(az._hold_hue, az._hold_sat), trans=0.25)
        return
    ...
    if mode == "pulse":
        if beat > 0.5:
            ...
            for e in targets:
                if az.ok(e, b, 0, 0, max(0.9, iv * 2)):
                    ha.set_lights([e], b, hs=(0, 0), trans=0.2)
        return
            ...
            for e in bands["bass"]:
                if az.ok(e, bright(bass), 5, 80, min_iv, force=beat > 0.5):
                    ha.set_lights([e], bright(bass) if beat < 0.5 else max_b,
                                  ...
                                  trans=0.0 if beat > 0.5 else 0.15)
        if bands["mid"]:
            for e in bands["mid"]:
                if az.ok(e, bright(mid), 130, 70, min_iv * 1.5):
                    ha.set_lights([e], bright(mid), hs=(130, 70), trans=0.2)
        if bands["high"]:
            for e in bands["high"]:
                if az.ok(e, bright(high), 250, 65, min_iv * 1.5):
                    ha.set_lights([e], bright(high), hs=(250, 65), trans=0.2)
        if mode == "spectrum" and bands["full"]:
            hue = (mid * 120 + high * 240) % 360
            for e in bands["full"]:
                if az.ok(e, bright(amp), hue, 55, min_iv * 1.5):
                    ha.set_lights([e], bright(amp), hs=(hue, 55), trans=0.2)
        return

    if mode == "color_cycle":
        az.hue = (az.hue + 3) % 360
        for e in targets:
            if az.ok(e, bright(max(0.3, amp)), az.hue, 70, max(1.0, iv * 2)):
                ha.set_lights([e], bright(max(0.3, amp)), hs=(az.hue, 70), trans=0.3)
        return

    if mode == "cinema":
        for e in targets:
            if az.ok(e, bright(0.3 + bass * 0.4), 30, 65, max(1.2, iv * 3)):
                ha.set_lights([e], bright(0.3 + bass * 0.4), hs=(30, 65), trans=0.4)
        return

    for e in targets:
        if az.ok(e, bright(max(0.3, amp)), 0, 0, max(0.8, iv * 2)):
            ha.set_lights([e], bright(max(0.3, amp)), hs=(0, 0), trans=0.2)
...
def reload_cfg():
    ...
    UPDATE_MS = max(int(o.get("update_interval_ms", UPDATE_MS)), 250)
