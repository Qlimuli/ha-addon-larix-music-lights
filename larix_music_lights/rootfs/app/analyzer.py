...
log.info(
    "Config loaded: lights=%s profiles=%d options_keys=%s",
    LEGACY_LIGHTS,
    len(PROFILES),
    list(_OPTIONS.keys()) if _OPTIONS else [],
)
...
def main():
    ...
    state.log_event(
        f"Started - profile '{profile.get('name')}' mode '{mode}' lights={all_entities}",
        "success",
    )
    ...
    log.info(
        "Active: profile=%s mode=%s sensitivity=%.2f lights=%s",
        profile.get("name"),
        mode,
        sensitivity,
        all_entities,
