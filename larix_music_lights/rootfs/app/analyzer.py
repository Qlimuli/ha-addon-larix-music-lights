#!/usr/bin/env python3
"""
Larix Music Reactive Lights - Audio analyzer & light controller (v1.3.1)
...
log.info("Config loaded: lights=%s profiles=%d options_keys=%s",
         LEGACY_LIGHTS, len(PROFILES), list(_OPTIONS.keys()) if _OPTIONS else [])
...
def load_active_profile() -> Dict[str, Any]:
    ...
    log.info(
        "No profiles defined - using legacy light_entities (%d) / area_ids (%d)",
        len(LEGACY_LIGHTS),
        len(LEGACY_AREAS),
    )
    return {
...
class HomeAssistant:
    ...
    def set_lights(
        ...
        if result is None and hs_color is not None:
            payload.pop("hs_color", None)
...
def resolve_band_lights(ha: HomeAssistant, profile: Dict[str, Any]) -> Dict[str, List[str]]:
    ...
    if not any([bass, mid, high, full]):
        log.error(
            ...
            "LEGACY_LIGHTS=%s OPTIONS_KEYS=%s",
            LEGACY_LIGHTS,
            list(_OPTIONS.keys()) if _OPTIONS else "(no options.json)",
        )
...
def main():
    ...
    if _HAS_WEBUI:
        ...
        state = _NullState()
        log.warning("Running without Watchdog GUI")
    ...
    state.log_event(f"Started - profile '{profile.get('name')}' mode '{mode}' lights={all_entities}", "success")
    ...
    log.info("Active: profile=%s mode=%s sensitivity=%.2f lights=%s",
             profile.get("name"), mode, sensitivity, all_entities)
